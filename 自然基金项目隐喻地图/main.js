...
window.onload = function () {
    ...
    var detailLayers = svg.append('g')  // 包含各细节图层的g
            .attr("id", "detailLayer"),
        baseLayers = svg.append("g");//包含各基本图层的g
    var g1 = baseLayers.append("g"),//第一层
        dG1 = detailLayers.append("g");
    g1.level = 1, g1.id = "g1";

    var g2 = baseLayers.append("g"),//第二层
        dG2 = detailLayers.append("g");
    g2.level = 2;

    var g3 = baseLayers.append("g"),//第三层
        dG3 = detailLayers.append("g");
    g3.level = 3;

    var g4 = baseLayers.append("g"),//第四层
        dG4_1 = detailLayers.append("g"),//最后一个detailLayer有两种状态，故用两个g元素来存储
        dG4_2 = detailLayers.append("g");
    ...
    dGroups = [dG1, dG2, dG3, dG4_1, dG4_2];

    var detail_g = baseLayers.append("g")//点击某个多边形放大后，展示其内部细节的容器
    ...
    function zoomed() {
        ...
        detailLayers.style("stroke-width", 1.5 / d3.event.scale.toFixed(3) + "px");  //1.5
        detailLayers.attr("transform", "translate(" + d3.event.translate + ")scale(" + d3.event.scale + ")");
    ...
    function focus(d, current_this) {
        {
            ...
            //current_group.style("opacity", "0.9");//0.8
...
//根据缩放倍数显示不同细节图层
    function LOD(scale, valid) {
        ...
        if (valid) {
            ...
            if (showDetailLayer === true && scale > current_group.maxScale) {
                active.style("visibility", "hidden");//鼠标点击区域隐藏（这样鼠标才能悬浮在下一图层上）
                if (scale - 1.5 <= 0.4 && scale - 1.5 >= -0.2)   //范围 1.3-1.9
                {
                    detailHide();
                    dGroups[0].style("display", "");
                }
                if (scale - 2.0 <= 0.7 && scale - 2.0 >= -0.1)   // 1.9-2.7
                {
                    detailHide();
                    dGroups[1].style("display", "");
                }
                if (scale - 3.0 <= 2.2 && scale - 3.0 >= -0.3)      // 2.7-5.2
                {
                    detailHide();
                    dGroups[2].style("display", "");
                }
                if (scale > 5.2) {
                    detailHide();
                    if (current_level < 4) {
                        dGroups[3].style("display", "");//显示dg4_1
                    }
                    else {
                        dGroups[4].style("display", "");//显示dg4_2
                    }
                }
            }
            if (scale <= current_group.mScale) {
                detailHide();
                active.style("visibility", "visible");
...
//鼠标点击已激活多边形区域或者背景矩形区域，恢复到初始状态
    ...
    function back2browse() {
        ...
        // current_group.style("visibility","visible");
...
function buildNeighbor() {
    ...
    for (var i = 0, len = departments.length; i < len; i++) {
        ...
        if (name !== "地球科学部") {   //name==="工程与材料科学部" || name==="生命科学部" || name==="化学科学部"
